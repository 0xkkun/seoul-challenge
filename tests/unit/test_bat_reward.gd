...
func test_default_is_barehands_then_bat() -> void:
	...
	_runner.assert_eq(p.current_weapon_name(), "금 간 알루미늄 배트", "보상 후 일반 배트 이름")
	p.free()
...
func test_equip_bat_disables_baseball_ranged_mode() -> void:
	...
	_runner.assert_eq(p.current_weapon_name(), "금 간 알루미늄 배트", "bat equip still updates weapon name")
	p.free()
