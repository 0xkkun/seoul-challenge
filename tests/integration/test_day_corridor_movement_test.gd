...
func test_day_corridor_onboarding_reward_dialogue_grants_bat_and_clue() -> void:
	...
	_runner.assert_true(scene.get_active_dialogue_text().contains("금 간 나무 배트"), "captain names the cracked wooden bat before the reward is claimed")
...
func test_day_corridor_onboarding_reward_bat_line_shows_cracked_bat_pickup_popup() -> void:
	...
	if dialogue_ui.get_unlock_items().size() == 1:
		_runner.assert_eq(dialogue_ui.get_unlock_items()[0]["name"], "금 간 나무 배트", "bat pickup popup uses the pre-reward cracked bat name")
