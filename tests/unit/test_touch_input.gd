...
func test_ingame_control_onboarding_reveals_real_skip_escape_and_keeps_compact_legend() -> void:
	...
	var legend_snapshot: Dictionary = onboarding.call("get_compact_legend_snapshot")
	_runner.assert_eq(legend_snapshot.get("input_mode"), &"desktop", "PC compact 조작표는 desktop 입력 언어를 쓴다")
	_runner.assert_eq(legend_snapshot.get("input_actions", []), [&"move", &"attack", &"dash"], "PC compact 조작표는 이동·타격·회피 글리프를 남긴다")
	_runner.assert_eq(legend_snapshot.get("labels", []), ["이동", "타격", "회피"], "각 글리프 옆에는 짧은 한국어 행동어를 붙인다")
	var legend_copy := String(legend_snapshot.get("visible_copy", ""))
	_runner.assert_false("LMB" in legend_copy or "RMB" in legend_copy or "SPACE" in legend_copy, "compact 조작표에도 개발자 입력 약어를 노출하지 않는다")
...
func test_ingame_control_onboarding_desktop_guidance_names_keyboard_inputs_without_touch_targets() -> void:
	...
	_runner.assert_eq(snapshot.get("input_actions", []), [&"move"], "첫 안내는 WASD 글리프 묶음을 쓴다")
	_runner.assert_eq(snapshot.get("key_label"), "", "PC 이동 안내는 text key chip을 숨긴다")
...
func test_ingame_control_onboarding_desktop_actions_match_pc_control_scheme() -> void:
	...
	_runner.assert_eq(attack_snapshot.get("input_actions", []), [&"attack"], "기본공격 안내는 좌클릭 그림을 쓴다")
	_runner.assert_eq(attack_snapshot.get("key_label"), "", "기본공격 안내는 LMB text를 숨긴다")
	_runner.assert_eq(attack_snapshot.get("action"), "타격", "기본공격 행동어가 짧다")
	onboarding.call("record_action", &"attack_executed")
	var dash_snapshot: Dictionary = onboarding.call("get_current_step_snapshot")
	_runner.assert_eq(dash_snapshot.get("input_actions", []), [&"dash"], "대시 안내는 Shift 키캡을 쓴다")
	_runner.assert_eq(dash_snapshot.get("key_label"), "", "대시 안내는 SPACE text를 숨긴다")
	...
	_runner.assert_eq(power_snapshot.get("input_actions", []), [&"dash", &"attack"], "강공격 안내는 Shift 뒤 좌클릭 그림 순서를 쓴다")
	_runner.assert_eq(power_snapshot.get("key_label"), "", "강공격 안내는 key 약어 조합을 숨긴다")
	_runner.assert_eq(power_snapshot.get("action"), "강하게 타격", "강공격 행동어가 짧다")
