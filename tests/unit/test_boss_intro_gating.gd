...
func test_first_encounter_plays_onboarding_intro() -> void:
	...
	_runner.assert_true(String(beats[2]["text"]).contains("마지막 시즌의 배트"), "보스 첫 조우는 주장 보상 후 배트 이름을 사용한다")
	_runner.assert_false(String(beats[2]["text"]).begins_with("그 "), "보스 첫 조우는 어색한 지시어 없이 배트 이름을 말한다")
