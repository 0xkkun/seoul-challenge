...
func test_session_starts_control_onboarding_only_for_first_baseball_run() -> void:
	...
	if onboarding != null:
		...
		if onboarding.has_method("get_current_step_snapshot"):
			...
			_runner.assert_eq(snapshot.get("key_label"), "", "desktop session hides the text-only movement key chip")
			_runner.assert_eq(snapshot.get("input_actions", []), [&"move"], "desktop session renders the WASD movement glyph cluster")
